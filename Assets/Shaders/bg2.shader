    ...
    Shader "ShaderMan/NonEpilepsyInducing2DWorldShaderWhatCanITypeToMakeThisALongerNameIGuessThisIsSufficientLikeAndSubscribe"
	{
	Properties{
...
for(int y = -2; y<=2;y++) {
            ...
            m += smoothstep(r*1.00001, r, d);
        }
    }
    
    col.x = fmod(m/1., 1.1) / 1.25;
    col.y = fmod(m/1.1, 1.3) / 2.0;
